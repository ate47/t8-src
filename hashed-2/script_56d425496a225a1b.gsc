	...
	else
	{
		...
		if(isdefined(einflictor) && isvehicle(einflictor) && issentient(einflictor))
		{
			if(is_valid_target(einflictor, self.team))
			{
				...
				self namespace_14c38db0::function_a4d6d6d8(einflictor);
			}
		}
		else if(isalive(eattacker) && issentient(eattacker) && (!(isplayer(eattacker) && eattacker isremotecontrolling())) && is_valid_target(eattacker, self.team))
		{
			...
			self namespace_14c38db0::function_a4d6d6d8(eattacker);
		}
	}
	return idamage;
}
...
function turretscanning()
{
	...
	while(true)
	{
		...
		if(isdefined(veh.enemy))
		{
			...
			var_2aa33bf1 = 0;
			if(distancesquared(veh.enemy.origin, veh.origin) > veh.var_38e6355c && veh.var_7eb3ebd5[veh.enemy getentitynumber()] === #"hash_704614cf51e8952f")
			{
				var_2aa33bf1 = 1;
			}
			...
			if(!isdefined(veh.var_7eb3ebd5[veh.enemy getentitynumber()]) && veh namespace_14c38db0::function_1c169b3a(veh.enemy))
			{
				veh.var_c8072bcc = gettime();
				veh.var_7eb3ebd5[veh.enemy getentitynumber()] = #"hash_704614cf51e8952f";
			}
		}
...
/*
	...
	Flags: Linked
*/
function function_9a2c2b78(killstreakid, team)
{
	turret = self;
...
function watchturretshutdown(player, killstreakid, team)
{
	...
	function_9a2c2b78(killstreakid, team);
}
...
function function_d6c5b32b(player, killstreakid, team)
{
	...
	if(isdefined(turret))
	{
		turret function_9a2c2b78(killstreakid, team);
	}
}
